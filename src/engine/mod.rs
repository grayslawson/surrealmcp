...
impl Response {
    ...
    pub fn into_mcp_result(mut self) -> Result<rmcp::model::CallToolResult, rmcp::ErrorData> {
        if let Some(res) = self.result.as_mut() {
            ...
            let value: surrealdb::types::Value = res.take(0).map_err(|e: surrealdb::Error| {
...
mod tests {
    ...
    async fn test_complex_type_validation() {
        ...
        if let rmcp::model::RawContent::Text(text_content) = &mcp_result.content[0].raw {
            let _json: serde_json::Value = serde_json::from_str(&text_content.text)
                .map_err(|e| {
                    rmcp::ErrorData::internal_error(format!("Failed to parse JSON: {}", e), None)
                })
            ...
            assert!(
                text_content.text.contains("sector"),
                "Missing 'sector' in ID"
            );
            ...
            // Verify Duration
            assert!(
                text_content.text.contains("5s")
                    || text_content.text.contains("duration")
                    || text_content.text.contains("secs"),
