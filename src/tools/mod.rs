...
use crate::resources;
use crate::utils::{
    convert_json_to_surreal, is_safe_surrealql_snippet, parse_target, parse_targets,
};
...
"#)]
    pub async fn select(
        ...
        if let Some(v) = where_clause {
            if !is_safe_surrealql_snippet(&v) {
                return Err(McpError::internal_error(
                    "Invalid where clause: contains unquoted semicolon".to_string(),
                    None,
                ));
            }
            query.push_str(&format!(" WHERE {v}"));
        }
        // Add the split clause if provided
        if let Some(v) = split_clause {
            if !is_safe_surrealql_snippet(&v) {
                return Err(McpError::internal_error(
                    "Invalid split clause: contains unquoted semicolon".to_string(),
                    None,
                ));
            }
            query.push_str(&format!(" SPLIT ON {v}"));
        }
        // Add the group clause if provided
        if let Some(v) = group_clause {
            if !is_safe_surrealql_snippet(&v) {
                return Err(McpError::internal_error(
                    "Invalid group clause: contains unquoted semicolon".to_string(),
                    None,
                ));
            }
            query.push_str(&format!(" GROUP BY {v}"));
        }
        // Add the order clause if provided
        if let Some(v) = order_clause {
            if !is_safe_surrealql_snippet(&v) {
                return Err(McpError::internal_error(
                    "Invalid order clause: contains unquoted semicolon".to_string(),
                    None,
                ));
            }
        ...
        if let Some(v) = limit_clause {
            if !is_safe_surrealql_snippet(&v) {
                return Err(McpError::internal_error(
                    "Invalid limit clause: contains unquoted semicolon".to_string(),
                    None,
                ));
            }
            query.push_str(&format!(" LIMIT BY {v}"));
        }
        // Add the start clause if provided
        if let Some(v) = start_clause {
            if !is_safe_surrealql_snippet(&v) {
                return Err(McpError::internal_error(
                    "Invalid start clause: contains unquoted semicolon".to_string(),
                    None,
                ));
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_tool_select_injection_rejection() {
        let service = setup_service().await;

        // Attempt injection in where_clause
        let res = service
            .select(Parameters(SelectParams {
                targets: vec!["person".to_string()],
                where_clause: Some("age > 18; DELETE person".to_string()),
                split_clause: None,
                group_clause: None,
                order_clause: None,
                limit_clause: None,
        ...
        assert!(
            err.message
                .contains("Invalid where clause: contains unquoted semicolon")
        );
    }
}
