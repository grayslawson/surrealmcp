        ...
        if let Some(v) = where_clause {
            if !is_safe_surrealql_snippet(&v) {
                return Err(McpError::invalid_params(
                    "Invalid clause: unsafe SurrealQL snippet detected",
                    None,
                ));
            }
...
"#)]
    pub async fn relate(
        ...
        // Validate the relationship table name
        if !is_safe_surrealql_snippet(&table) {
            return Err(McpError::invalid_params(
                "Invalid relationship type: unsafe SurrealQL snippet detected",
                None,
            ));
        }
        // Process the tables and Record IDs
...
mod tests {
    ...
    async fn test_tool_query_simple() {
        ...
        match &res.content[0].raw {
            rmcp::model::RawContent::Text(raw_text) => assert!(raw_text.text.contains("42")),
